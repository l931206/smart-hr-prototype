(function () {
  ...
  async function approvalDetail() {
    ...
    [item.employee_department || "—", item.leave_type, `${item.start_date} 至 ${item.end_date}`, `${item.days} 天`, item.reason || "—", "未上傳附件"].forEach((value,index) => { if(values[index]) values[index].textContent=value; });
  ...
  async function announcementResult() {
    ...
    const values = document.querySelectorAll(".info .row strong, .announcement-info .row strong");
    const data = [item.title, `ANN-${item.id}`, item.category_label, item.is_published ? "已發布" : "草稿"];
    data.forEach((value,index) => { if(values[index]) values[index].textContent=value; });
  ...
  async function announcementEdit() {
    ...
    card.querySelector("form").addEventListener("submit", async (event) => { event.preventDefault(); await apiRequest(`/announcements/${item.id}/`, {method:"PATCH",body:JSON.stringify({title:card.querySelector("#editAnnouncementTitle").value.trim(),content:card.querySelector("#editAnnouncementContent").value.trim(),is_published:card.querySelector("#editAnnouncementPublished").value === "true"})}); location.href=`announcement-edit-success.html?id=${item.id}`; });
  }
